...
app.post('/checkout', (req, res) => {
  ...
  newOrder.save()
    .then(order => {
      ...
      client.messages.create({
        body: `New Order Received:\n${orderDetails}`,
        to: process.env.TO_PHONE_NUMBER_5,
        from: process.env.FROM_PHONE_NUMBER
      })
...
app.post('/cancel-order', (req, res) => {
  ...
  Order.findByIdAndUpdate(id, { status: 'canceled' }, { new: true })
    .then(updatedOrder => {
      if (!updatedOrder) {
...
app.get('/orders', (req, res) => {
  ...
  Order.find({ status: 'pending' })
    .then(pendingOrders => {
      // Luego, buscar los pedidos completados
      Order.find({ status: 'completed' })
        .then(completedOrders => {
          // Por último, buscar los pedidos cancelados
          Order.find({ status: 'canceled' })
            .then(canceledOrders => {
              ...
              // Renderizar pedidos cancelados
              canceledOrders.forEach((order, index) => {
                html += `
                  <li class="order-item canceled">
                    <h2>Pedido ${index + 1}</h2>
                    <p><strong>Nombre:</strong> ${order.name}</p>
                    <p><strong>Dirección:</strong> ${order.address}</p>
                    <p><strong>Teléfono:</strong> ${order.phoneNumber}</p>
              ...
              html += `
                    ...
                    <script>
                      ...
                      document.getElementById('authForm').addEventListener('submit', function(event) {
                        ...
                        // Aquí puedes validar tus credenciales
                        // Por ejemplo, usando variables de entorno u otro método:
                      ...
                      // Completar pedido (cambia el estado a "completed")
                      ...
                      // Cancelar pedido (cambia el estado a "canceled")
